...
const { readStdinJson, outputJson, log, resolveWorkspaceRoot } = require('../lib/utils');
...
function runVerificationChecks(workspaceRoot) {
  ...
  for (const check of checks) {
    try {
      log(`Running pre-push check: [${check.name}]...`);
...
async function main() {
  ...
  log(`Detected 'git push' command: "${commandLine}". Triggering pre-push verification...`);
  const workspaceRoot = resolveWorkspaceRoot();
  const result = runVerificationChecks(workspaceRoot);

  if (!result.passed) {
    log(`Pre-push verification FAILED on [${result.failedCheck}]. Blocking git push.`);
    return outputJson({
      decision: 'deny',
      reason: [
  ...
  outputJson({
    decision: 'allow',
    reason: 'Pre-push verification checks (build, typecheck, lint) passed successfully.',
  });
}
